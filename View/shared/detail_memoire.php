...
<a href="liste_memoires.php" style="font-size:.75rem;color:var(--navy);display:inline-flex;align-items:center;gap:.3rem;margin-bottom:.8rem;">← Retour à la liste</a>
    ...
    <div class="memoire-content" style="background:linear-gradient(135deg,#061640,#0a2463,#1a3a7a);border-radius:var(--radius);padding:1.2rem 1.4rem;color:#fff;margin-bottom:1rem;position:relative;overflow:hidden;">
      ...
      <div style="display:flex;gap:.5rem;margin-top:.8rem;align-items:center;flex-wrap:wrap;">
        ...
        <form method="POST" action="../../Controller/traitement_auth.php" style="display:inline;">
          ...
          <input type="hidden" name="action_like" value="<?= $aLike ? 'unlike' : 'like' ?>">
          <button type="submit" style="background:<?= $aLike ? 'rgba(239,68,68,.25)' : 'rgba(255,255,255,.15)' ?>;color:<?= $aLike ? '#fff' : 'rgba(255,255,255,.9)' ?>;border:1px solid rgba(255,255,255,.3);border-radius:20px;padding:6px 14px;font-size:.75rem;cursor:pointer;font-weight:600;display:inline-flex;align-items:center;gap:.4rem;transition:all .2s;">
            <i class="fas fa-heart" style="<?= $aLike ? 'color:#ef4444' : '' ?>"></i> <?= $memoire['nbLikes'] ?> <?= $aLike ? "Retirer" : "J'aime" ?>
          </button>
        </form>
    ...
    <div class="card">
      ...
      foreach ($commentaires as $c) {
          $parent = $c['idParent'] ?? null;
          $byParent[$parent ?? 'root'][] = $c;
      }

      function render_comments(array $list, array $byParent, int $level = 0) {
          foreach ($list as $c) {
              $indent = $level * 16;
              ?>
              <div style="display:flex;gap:.8rem;margin-bottom:1.2rem;margin-left:<?= $indent ?>px;">
                ...
                <div style="flex:1;">
                  <div style="background:var(--gray-50);border-radius:12px;padding:.8rem 1rem;">
                    ...
                    <div style="font-size:.85rem;color:var(--gray-700);line-height:1.5;"><?= nl2br(htmlspecialchars($c['contenu'])) ?></div>
                  </div>
                  <div style="display:flex;gap:1rem;margin-top:.4rem;margin-left:.5rem;">
                    <button class="btn-like-comment" data-id="<?= $c['idCommentaire'] ?>" style="background:none;border:none;color:var(--gray-500);font-size:.75rem;font-weight:600;cursor:pointer;padding:0;">J'aime</button>
                    <button class="btn-reply" data-id="<?= $c['idCommentaire'] ?>" style="background:none;border:none;color:var(--gray-500);font-size:.75rem;font-weight:600;cursor:pointer;padding:0;">Répondre</button>
                  </div>
                  <div class="reply-form" id="reply-<?= $c['idCommentaire'] ?>" style="display:none;margin-top:.6rem;margin-left:.5rem;">
                    <form method="POST" action="../../Controller/traitement_auth.php">
                      <input type="hidden" name="idMemoire" value="<?= $memoire['idMemoire'] ?>">
                      <input type="hidden" name="parentId" value="<?= $c['idCommentaire'] ?>">
                      <textarea name="contenu" rows="2" placeholder="Écrivez une réponse..." style="resize:none;border-radius:8px;padding:.5rem 8px;width:100%;" required></textarea>
                      <div style="margin-top:.4rem;">
                        <button class="btn btn-primary btn-sm" type="submit" name="btn_commenter" style="border-radius:12px;">Répondre</button>
                        <button type="button" class="btn-cancel-reply" data-id="<?= $c['idCommentaire'] ?>" style="margin-left:.5rem;border-radius:12px;">Annuler</button>
                      </div>
                    </form>
                  </div>
                </div>
              ...
              <?php
              $children = $byParent[$c['idCommentaire']] ?? [];
              if (!empty($children)) {
                  render_comments($children, $byParent, $level + 1);
              }
          }
      }

      $top = $byParent['root'] ?? [];
      render_comments($top, $byParent, 0);
      ?>
      <script>
      document.addEventListener('click', function(e){
        if (e.target.matches('.btn-reply')) {
          var id = e.target.getAttribute('data-id');
          document.querySelectorAll('.reply-form').forEach(function(el){ el.style.display = 'none'; });
          var f = document.getElementById('reply-' + id);
          if (f) f.style.display = 'block';
        }
        if (e.target.matches('.btn-cancel-reply')) {
          var id = e.target.getAttribute('data-id');
          var f = document.getElementById('reply-' + id);
