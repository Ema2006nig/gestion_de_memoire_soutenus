...
class CommentaireRepository {
    ...
    public static function create(string $idCompte, string $idMemoire, string $contenu, float $note = 0, ?string $idParent = null): string {
        $pdo = getConnexion();
        $stmt = $pdo->query("SELECT COUNT(*) FROM commentaire");
        $count = $stmt->fetchColumn();
        $idCommentaire = 'COM' . str_pad($count + 1, 4, '0', STR_PAD_LEFT);
        
        $stmt = $pdo->prepare("INSERT INTO commentaire (idCommentaire, idCompte, idMemoire, idParent, contenu, note, dateCommentaire) VALUES (?, ?, ?, ?, ?, ?, NOW())");
        $stmt->execute([$idCommentaire, $idCompte, $idMemoire, $idParent, $contenu, $note]);
    ...
    public static function getByMemoire(string $idMemoire): array {
        $pdo = getConnexion();
        $stmt = $pdo->prepare("SELECT co.*, u.nom, u.prenom, u.role
            FROM commentaire co
            JOIN compte c ON co.idCompte = c.idCompte
            JOIN utilisateur u ON c.idUtilisateur = u.idUtilisateur
            WHERE co.idMemoire = ?
            ORDER BY co.dateCommentaire ASC");
